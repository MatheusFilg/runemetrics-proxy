...
interface getPlayerDetailsProps {
  ...
  export async function getPlayerDetailsService({ name, activities }: getPlayerDetailsProps): Promise<PlayerDetailsResponse> {
    try {
      ...
      if (axios.isAxiosError(error)) {
        console.error(`Erro na API do RuneScape: ${error.response?.status} - ${error.message}`);
